...
void film::CompareFrame (AVFrame * pFrame, AVFrame * pFramePrev)
{
  ...
  int prev_frame_number = shots.back().fbegin;
  if (diff > this->threshold && score > this->threshold && frame_number > prev_frame_number + this->gop) {
    shot s;
...
void film::shotlog (double value)
{
  ...
  if (display) {
    wxString mess;
    mess = wxString (wxString::FromAscii (str.c_str ()));
    wxMutexGuiEnter ();
    wxMessageDialog MsgDlg (dialogParent, mess);
    MsgDlg.ShowModal ();
    wxMutexGuiLeave ();
